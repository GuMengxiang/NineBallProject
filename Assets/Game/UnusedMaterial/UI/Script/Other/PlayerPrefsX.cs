...
public class PlayerPrefsX
{
    ...
    public static void SaveFloatListToArray(List<float> inList, string key)
    {
        float[] valueArray = new float[inList.Count];

        for (int i = 0; i < inList.Count; i++)
        {
            valueArray[i] = inList[i];
        }

        PlayerPrefsX.SetFloatArray(key, valueArray);
    }

    public static void PopulateFloatListFromArray(ref List<float> inList, string key)
    {
        float[] inArray = GetFloatArray(key);

        if (inArray != null && inArray.Length > 0)
        {
            inList = new List<float>(0);

            for (int i = 0; i < inArray.Length; i++)
            {
                inList.Add(inArray[i]);
            }
        }
        else
        {
