...
public class AnimationSpline
{
	...
	{
		Vector3 newPosition = this.Evaluate(step + speed * Time.fixedDeltaTime);
		slider.position = this.Evaluate(step);
        step += sliderOrient * speed * Time.fixedDeltaTime;

        if (step + speed * Time.fixedDeltaTime >= splineLength)
        {
            velocity = (1.0f / Time.fixedDeltaTime) * (slider.position - this.Evaluate(step - speed * Time.fixedDeltaTime));
        }
		else
        {
            velocity = (1.0f / Time.fixedDeltaTime) * (newPosition - slider.position);
        }
